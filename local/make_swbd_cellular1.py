...
if len(sys.argv) != 3:
    raise Exception("Usage: {0} <path-to-LDC2001S13> <path-to-output>".format(os.path.basename(__file__)))

db_base, outdir = str(sys.argv[1]), str(sys.argv[2])

subprocess.call('mkdir -p ' + outdir, shell=True)

tmp_dir = 'data/tmp_cell1'
...
CS = open(db_base + '/swb_cell_1_audio_d1/doc/swb_callstats.tbl', 'r')
